function [U_k,R_k,Y_k,w_pU,w_pR,w_pY]=func_possibilistic_is(U_k_id,R_k_id,Y_k_id,Ns,k_max,ling_x,ling_aggrfx,intpair_x,intpair_aggrfx,LB_R,UB_R,LB_UY,UB_UY)
...
if ~isempty(R_k_id)
    for j=1:length(R_k_id)
        R_k{1}=LB_R(R_k_id)'+rand(length(R_k_id),Ns).*UB_R(R_k_id)';
    end
end
...
% Define U / estimable w input variables (does not matter the length of U_k_id, it automaticly replaces the relevant values)
w_feed=w_feed_0; 
if U_k_id>0
...
if ~isempty(R_k_id)
for j=1:length(R_k_id)
[R_k{k+1}(j,:)]=resample(R_k{k}(j,:),w_all{k},'multinomial_resampling'); %P
end
end
end 
end
